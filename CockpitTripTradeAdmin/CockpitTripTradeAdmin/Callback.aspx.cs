...
namespace CockpitTripTradeAdmin
{
    public partial class Callback : System.Web.UI.Page
    {
        ...
        private void VerifyLogin()
        {
            ...
            {
                ErrorHandling("State not equal", "Origin State: " + sessionState + ", Return State: " + state);
            }
            ...
            else
            {
                ...
                else
                {
                    ...
                    //************* Create User Session *************
                    LoginSession.SetLoginSession(sessionState, it.sub, WebApplicationEnum.CockpitTripTradeAdmin);
                    ...
                    else
                    {
                        if (!string.IsNullOrEmpty(ls.Employee.EmployID) && ls.IsSucceed)
                        {
                            ...
                            // 20221108 648267:若為Admin > 使用ImpersonateUser；若為一般後台人員 > 跳轉到"審核申請單"；若為OGAdmin > 轉跳到"查詢申請單"
                            if (ls.IsAdmin)
                            {
                                Response.Redirect("~/Module/Admin/ImpersonateUser.aspx");
                                //Response.Redirect("~/Module/Application/ReviewFcdsApply_List.aspx");
                            }
                            else if (!CheckIsOGGroup(ls)) 
                            {   
                                Response.Redirect("~/Module/Application/ReviewFcdsApply_List.aspx");
                            }
                            else
                            {
                                Response.Redirect("~/Module/Application/FcdsApply_List.aspx");
                            }
                        }
                        else
                        {
        ...
        /// <summary>
        /// 檢查是否為OG群組
        /// 20221108 648267:新增檢查是否為OG群組，若為OG頁面轉跳到[查詢]
        /// </summary>
        /// <param name="ls"></param>
        /// <returns>bool(是否為OG群組內的人員)</returns>
        private bool CheckIsOGGroup(LoginSession ls)
        {
            return Group.IsOGAdmin(ls.UserGroups);
        }
        ...
        private bool SaveOIDCErrorHandlingLog(string logSubject, string logDetail)
        {
            Log log = new Log()
            {
                WebApplication = Enum.TryParse<WebApplicationEnum>(RequestUtility.Url.Segments[1].Replace("/", ""), out _) ? (Enum.Parse(typeof(WebApplicationEnum), RequestUtility.Url.Segments[1].Replace("/", "")) as WebApplicationEnum?).ToString() : RequestUtility.Url.Segments[1].Replace("/", "").ToString(),
