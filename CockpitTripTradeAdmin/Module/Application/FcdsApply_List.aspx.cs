...
{
    public partial class CockpitTripTradeAdmin_Module_Application_FcdsApply_List : ApplicationPage
    {
        private ModuleForm moduleForm;

        protected void Page_Load(object sender, EventArgs e)
        {
            this.BaseFormTitle = @"查詢申請單";
        ...
        private void InitForm()
        {
            ...
            this.ddlStatusCode.DataSource = FlowStatus.FetchAllFlowStatus(moduleForm.ModuleName, moduleForm.FormName);
        ...
        {
            var col = (FcdsApply.FetchAll(ReturnObjectTypeEnum.Collection) as FcdsApplyCollection).ToList();
            ...
            if (dic.Count > 0)
            {
                ...
                {
                    if (key == @"AcType")
                        col = col.Where(o => o.IDAcTypeApplicant == dic[key]).ToList();
                    else if (key == @"CrewPos" && int.TryParse(dic[key], out _))
                        col = col.Where(o => o.IDCrewPosApplicant == int.Parse(dic[key])).ToList();
                    else if (key == @"ApplicationBeginDate" && DateTime.TryParse(dic[key], out _))
        ...
        private Dictionary<string, string> GetFilterDictionary()
        {
            ...
            if (this.ddlAcType.SelectedIndex != 0)
            {
                dic.Add(@"AcType", this.ddlAcType.SelectedValue);
            }

            if (this.ddlCrewPos.SelectedIndex != 0)
            {
                dic.Add(@"CrewPos", this.ddlCrewPos.SelectedValue);
            }
        ...
        private const int FLEET_INDEX = 2;
        private const int POS_INDEX = 3;
        private const int APPLICATION_DATE_INDEX = 5;
        private const int APPLICATION_DEADLINE_INDEX = 6;
        private const int APPLICANT_INDEX = 7;
        private const int RESPONDENT_INDEX = 8;
