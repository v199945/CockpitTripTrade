...
{
    public partial class CockpitTripTrade_Module_Application_FcdsApply : ApplicationPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            switch (this.BasePageModeEnum)
            {
                ...
                case PageMode.PageModeEnum.View:
                    HtmlGenericControl divContentBody = this.Master.FindControl(@"divContentBody") as HtmlGenericControl;
        ...
        private void BindForm()
        {
            ...
            //}
            // 20221108 V => 修改顯示[狀態]為中文+英文，若審批結束，則多顯示審批結果
            //this.StatusCode.Text = obj.DisplayStatus;
            string approvalResult = $"{obj.StatusCName}";
            if (obj.StatusCode == "RELEASED" && obj.IsApproval != null) 
            {
                if ( (bool)obj.IsApproval ) 
                {
                    approvalResult += $"-通過 ( Approved )";
                }
                else
                {
                    approvalResult += $"-拒絕 ( Disagreed )";
                }
            }
            else
            {
                approvalResult += $" ( {obj.StatusEName} )";
            }
            this.StatusCode.Text = approvalResult;
                ...
                case PageMode.PageModeEnum.Task:
                    switch (this.proID)
                    {
                        case FcdsHelper.PRO_RESPONDENT:
                            ...
                            //this.btnSave.Attributes.Add("OnClick", @"if (confirm('Are you sure to Submit to Crew Scheduling Dept?')) {window.onbeforeunload=null; " + this.BusyBox1.ShowFunctionCall + @" return true;} else{return false;}");
                            //this.btnReturn.Attributes.Add("OnClick", @"if (confirm('Are you sure to Return to [" + this.applicantCrewDb.ID + @" " + this.applicantCrewDb.PassportName + @"] ?')) {window.onbeforeunload=null; " + this.BusyBox1.ShowFunctionCall + @" return true;} else{return false;}");
                            break;
                        ...
                        case FcdsHelper.PRO_OP_STAFF:
                            this.checkApprove.Visible = false;
                            this.checkSave.Attributes.Add("class", "btn btn-primary-sp");
                            this.checkSave.InnerText = @"確認呈核 ( Confirm to submit )";
                            ...
                            this.btnApprove.Visible = false;
                            this.btnSave.CssClass = @"btn btn-primary-sp";
                            this.btnSave.Text = @"呈核 ( Submit for approval )";
                            this.btnReturn.Visible = false;
                            this.btnRevoke.Visible = false;

                            this.btnSave.Attributes.Add("OnClick", this.BusyBox1.ShowFunctionCall);
                            //this.btnSave.Attributes.Add("OnClick", @"if (confirm('Are you sure to Submit?')) {window.onbeforeunload=null; " + this.BusyBox1.ShowFunctionCall + @" return true;} else{return false;}");
                            break;
                        ...
                        case FcdsHelper.PRO_OP_GENERAL_MANAGER:
                            ...
                            this.checkSave.InnerText = @"確認呈核 ( Confirm to submit for approval )";
                            //this.spanSubmitTo.InnerText += @" " + emp.DepCDesc + @" " + emp.DisplayName + @" " + emp.PosCDesc + @"？";
                            this.checkReturn.Visible = false;
                            this.checkRevoke.Visible = false;
                            ...
                            this.btnSave.Attributes.Add("OnClick", this.BusyBox1.ShowFunctionCall);
                            //this.btnApprove.Attributes.Add("OnClick", @"if (confirm('Are you sure to Approve?')) {window.onbeforeunload=null; " + this.BusyBox1.ShowFunctionCall + @" return true;} else{return false;}");
                            //this.btnSave.Attributes.Add("OnClick", @"if (confirm('Are you sure to Submit?')) {window.onbeforeunload=null; " + this.BusyBox1.ShowFunctionCall + @" return true;} else{return false;}");
                            break;
                    }
                    break;
        ...
        private void BindUserControls()
        {
            ...
            this.ApprovalHistory1.ApprovalHistories = ApprovalHistory.GetApprovalHistoryCollection(this.BaseFormName, obj.FetchApprovalHistory(true));
            this.ApprovalHistory1.IsShowDurationField = false;

            // 依據 20210930 會議決議，前台組員檢視之申請單頁面移除[版本紀錄]頁籤。
        ...
        protected void btnApprove_Click(object sender, EventArgs e)
        {
            if (SaveObject(null, null, FcdsHelper.FcdsApplyStatusCodeEnum.RELEASED, true))
            {
                ...
                // 20221104 648267:送出後轉跳頁面到首頁
                Server.Transfer("TaskList.aspx");
                //string script = @"alert('Approve successfully!'); window.location='TaskList.aspx';";
        ...
        protected void btnSave_Click(object sender, EventArgs e)
        {
            ...
            // 20221110 648267:新增[被申請人]送出按鈕驗證
            // RESPONDENT check validation date
            if (fs.IDStatus == 2 && obj.SwappableBeginDate != null ) 
            {

                if ( obj.SwappableBeginDate < DateTime.Now ) 
                {
                    this.Alert("At least two working days prior to the earliest date of either duty is overdue.");
                    return;
                }  
            }
            ...
            if (SaveObject(fs.NextProID, nextTaskOwnerID, enumFcdsApplyStatusCode.Value, true))
            {
                ...
                // 20221104 648267:送出後轉跳頁面到首頁
                Server.Transfer("TaskList.aspx");
                //string script = @"alert('Submit successfully!'); window.location='TaskList.aspx';";
                //this.RunJavascript(script);
            }
            else
            {
        ...
        protected void btnRevoke_Click(object sender, EventArgs e)
        {
            ...
            {
                this.Alert($"Revoke failed, please try again.");
                Server.Transfer("TaskList.aspx");
            }
        ...
        protected void btnReturn_Click(object sender, EventArgs e)
        {
            ...
            {
                this.Alert("Reutrn failed, please try again.");
            }
        ...
        {
            this.RunJavascript(@"$('#checkSaveCompleteDialogLabel').text('" + buttonText + @" successfully'); $('#divSaveComplete').text('Form " + buttonText + @" successfully!'); $('#checkSaveCompleteDialogModal').modal('show');");
