...
namespace CockpitTripTrade.Module.Application.UserControl
{
    public partial class UserControl_FcdsApplyRoster : ApplicationUserControl
    {
        ...
        public void BindRepeaterList()
        {
            ...
            sw.Stop();
            Log log = Log.GetLogWithLoginSuccessfully(@"FcdsApplyRoster.repSchedule DataBind() cost", sw.Elapsed.TotalSeconds.ToString() + @" seconds", true);
            logger.Info(@"FcdsApplyRoster.repSchedule DataBind() Bind Repeater cost=" + sw.Elapsed.TotalSeconds + @" seconds");
            Debug.WriteLine(@"FcdsApplyRoster.repSchedule DataBind() Bind Repeater cost=" + sw.Elapsed.TotalSeconds + @" seconds");
            sw.Reset();
        ...
        protected void repSchedule_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            else if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (currItem != null)
                {
                    // 20221110 648267:新增換班隔月10天皆可勾選換(為了符合跨月班問題) 
                    DateTime nextMonthFirstDate = new DateTime(Int32.Parse(this.FcdsApply.SwapScheduleMonth.Substring(0, 4)),
                                                               Int32.Parse(this.FcdsApply.SwapScheduleMonth.Substring(4, 2)) + 1, 1);
                    bool nextMonthAvailableFlag = currItem.RosterDate >= nextMonthFirstDate && this.FcdsApply.DeadlineOfAcrossMonthDate.Value > DateTime.Now;
                    ...
                    if (divApplicantRoster != null)
                    {
                        ...
                        if (chkApplicantDate != null)
                        {
                            if (currItem.ApplicantRosterLegs.Count > 0 && currItem.ApplicantRosterLegs[0].CL_Order.HasValue && currItem.ApplicantRosterLegs[0].CL_Order == 0)
                            {
                                ...
                                //// 1. 判斷 Crew Route 第一個班表日期之月份和[互換月份]欄位值之月份相同
                                ///  2. 班表日期大於等於可申請換班起始日期(FcdsApply.SwappableBeginDate.Value)
                                //// 3. 且班表日期之航段集合物件(ApplicantRosterLegs)內有可換班之勤務(Is_Swappable)
                                //// 4. 且班表日期之航段集合物件(ApplicantRosterLegs)內無有跨月班之 Crew Route(Is_AcrossMonth)或跨月班申請期限日期(一般為申請月份的10號)大於當下日期
                                //// 5. 是則啟用可勾選，否則不啟用不可勾選
                        ...
                        divApplicantRoster.Attributes.Remove("class");
                        if (currItem.IsApplicantEnable || nextMonthAvailableFlag)
                        {
                    ...
                    if (divRespondentRoster != null)
                    {
                        ...
                        divRespondentRoster.Attributes.Remove("class");
                        if (currItem.IsRespondentEnable || nextMonthAvailableFlag)
                        {
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (item != null)
                {
                    ...
                    if (lblApplicantRpt != null && lblApplicantRptTime != null)
                    {
                        ...
                        {
                            // 20221101 648267:User要求不顯示有時間差的資料(eCrew和FCDS)
                            //lblApplicantRpt.Visible = true;
                            lblApplicantRpt.Visible = false;
                            lblApplicantRptTime.Visible = false;
                    ...
                    if (lblApplicantDbf != null && lblApplicantDbfTime != null)
                    {
                        if ((item.Leg_Flt > 0 || item.VCC_Debrief_Time > 0) && item.Duty_End_Dt_Tpe.HasValue)
                        {
                            ...
                            lblApplicantDbf.Visible = false;
                            lblApplicantDbfTime.Visible = false;
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (item != null)
                {
                    ...
                    if (lblRespondentRpt != null && lblRespondentRptTime != null)
                    {
                        ...
                        {
                            // 20221101 648267:User要求不顯示有時間差的資料(eCrew和FCDS)
                            //lblRespondentRpt.Visible = false;
                            lblRespondentRpt.Visible = false;
                            lblRespondentRptTime.Visible = false;
                    ...
                    if (lblRespondentDbf != null && lblRespondentDbfTime != null)
                    {
                        if ((item.Leg_Flt > 0 || item.VCC_Debrief_Time > 0) && item.Duty_End_Dt_Tpe.HasValue)
                        {
                            ...
                            lblRespondentDbf.Visible = false;
                            lblRespondentDbfTime.Visible = false;
