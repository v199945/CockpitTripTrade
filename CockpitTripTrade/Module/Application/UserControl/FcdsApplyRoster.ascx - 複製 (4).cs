...
namespace CockpitTripTrade.Module.Application.UserControl
{
    public partial class UserControl_FcdsApplyRoster : ApplicationUserControl
    {
        ...
        public void BindRepeaterList()
        {
            ...
            if (Page.IsPostBack)
            {
                if (this.BasePageModeEnum == PageMode.PageModeEnum.Create && !string.IsNullOrEmpty(this.RespondentCrewDb.ID))
                {
                    ...
                    //FcdsApplyRosterCollection col = ProcessRosterData(dtApplicant, dtRespondent);
                    ...
                    if (divApplicantRoster != null)
                    {
                        ...
                        if (chkApplicantDate != null)
                        {
                            if (currItem.ApplicantRosterLegs.Count > 0 && currItem.ApplicantRosterLegs[0].CL_Order.HasValue && currItem.ApplicantRosterLegs[0].CL_Order == 0)
                            {
                                ...
                                //// 1. 判斷 Crew Route 第一個班表日期之月份和[互換月份]欄位值之月份是否相同且班表日期是否大於等於可申請換班起始日期(FcdsApply.SwappableBeginDate.Value)
                                ...
                                currItem.IsApplicantEnable =
                                    ...
                                    || (currItem.ApplicantRosterLegs.Where(o => o.Is_Swappable == true).Count() > 0 && currItem.ApplicantRosterLegs.Where(o => o.Is_AcrossMonth == true).Count() > 0 && currItem.RosterDate > this.FcdsApply.DeadlineOfAcrossMonthDate.Value)
                                    ;
        ...
        public bool UpdateApplyRoster(FcdsApply fcdsApply, bool isSaveLog)
        {
            foreach (FcdsApplyRoster obj in this.FcdsApplyRosters)
            {
                ...
                //obj.ApplicantID = fcdsApply.ApplicantID;
                //obj.RespondentID = fcdsApply.RespondentID;
        ...
        //    foreach (RepeaterItem ri in this.repSchedule.Items)
        //    {
        //        col.Add(SetRepeaterItemValue(ri));
        //    }
            ...
            foreach (RepeaterItem ri in this.repSchedule.Items)
            {
                    ...
                    lblApplicantDate != null && !string.IsNullOrEmpty(lblApplicantDate.Text) && DateTime.TryParse(lblApplicantDate.Text, out _))
                {
                    ...
                    if (rosterDate.HasValue)
                    {
                        ...
                        {
                            //obj.IsApplicantEnable = divApplicantRoster.Attributes[@"class"].Contains(@"ss-selectable");
                            //obj.IsRespondentEnable = divRespondentRoster.Attributes[@"class"].Contains(@"ss-selectable");
