...
namespace Library.Component.DAL
{
    /// <summary>
    ...
    public sealed class OracleHelper
	{
        ...
        //private static readonly ILog logger = LogManager.GetLogger(typeof(OracleHelper));
		...
		{
            command.BindByName = true; // This property specifies the binding method in the collection. 
		...
		/// <summary>
		/// Execute an OracleCommand (that returns no resultset) with ArrayBindCount property against the database specified in the connection string 
		/// using the provided parameters.
		/// </summary>
		/// <remarks>
		/// e.g.:  
		///  int result = ExecuteNonQueryWithArrayBinding(connString, CommandType.StoredProcedure, "PublishOrders", new OracleParameter("@prodid", 24));
		/// </remarks>
		/// <param name="connectionString">a valid connection string for an OracleConnection</param>
		/// <param name="commandType">the CommandType (stored procedure, text, etc.)</param>
		/// <param name="commandText">the stored procedure name or PL/SQL command</param>
		/// <param name="rowsCount">the number of records to manipulation</param>
		/// <param name="commandParameters">an array of OracleParameters used to execute the command</param>
		/// <returns>an int representing the number of rows affected by the command</returns>
		public static int ExecuteNonQueryWithArrayBinding(string connectionString, CommandType commandType, string commandText, int rowsCount, params OracleParameter[] commandParameters)
		{
			//create & open an OracleConnection, and dispose of it after we are done.
			using (OracleConnection cn = new OracleConnection(connectionString))
			{
		...
		//private static void ExecuteWithArrayBinding<T>(OracleCommand cmd, Dictionary<string, OracleParameter> oraParams, Dictionary<string, PropertyInfo> props, T[] data)
		//{
